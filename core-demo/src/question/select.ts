...
class QSelect {
    data!:CFSelectQuestion;
    constructor() {}
    getMessage():string{
        let prev = '';
        const selectQuest = this.data;
        if(selectQuest.prev){
            prev = QTip.pre+','
        }
        let message = `${selectQuest.title}\n${QTip.quit},${prev}${QTip.next}\n`;
        if(selectQuest.optGroups){
            message += `请以-分割输入\n`;
            message += this.optGroupMessage(selectQuest.optGroups,0);
        }else{
            selectQuest.options.forEach((res:CFOption,index:number)=>{
